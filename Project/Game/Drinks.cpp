...
#include "DayByDay.h"
#include "HowToPlay.h"
Drinks::Drinks(rect3 rect, Player* _player, FoodType _type)
	: WorkBench(rect, _player), type(_type)
{
	food = new Food(type);
	timer = 0;
	position = vec2(rect.point1.x - 10, rect.point1.y);
	gauge_position = vec2(rect.point1.x, rect.point2.y + 10);
	texture = Engine::GetTextureManager().Load("assets/images/map/drinks.png", false);
...
void Drinks::Update([[maybe_unused]] double dt)
{
	deltaTime = dt;
