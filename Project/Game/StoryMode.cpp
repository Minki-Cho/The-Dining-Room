...
StoryMode::StoryMode() : guestTimer(0)
{
...
void StoryMode::Update(double dt)
{
	...
	if (Engine::GetInput().getPause() == false && GetGSComponent<Settings>()->GetOpenSettingScreen() == false)
	{
		...
		//theif
		if ((GetGSComponent<Days>()->GetNumber() == 4 || GetGSComponent<Days>()->GetNumber() == 8) && GetGSComponent<Timer>()->getTime() <= 35 && isTheif == false)
		{
		...
		if (GetGSComponent<Days>()->GetNumber() == 7 && GetGSComponent<DayByDay>()->GetDrawbool() == false)
		{
			// double gold event!
		...
		if (GetGSComponent<Days>()->GetNumber() == 10)
		{
			//Good Ending
			if (GetGSComponent<Number>()->GetNumber() >= 45
				&& GetGSComponent<Unlock>()->GetCheese() == true && GetGSComponent<Unlock>()->GetLettuce() == true && GetGSComponent<Unlock>()->GetTomato() == true 
				&& GetGSComponent<Unlock>()->GetIcecream() == true && GetGSComponent<Unlock>()->GetDash() == true && GetGSComponent<Reputation>()->getGauge() >= 90)
			{
				Engine::GetGameStateManager().SetNextState(static_cast<int>(Screens::GoodEnding));
			}

			//StoreTaken
			else if (GetGSComponent<Reputation>()->getGauge() >= 80)
			{
				Engine::GetGameStateManager().SetNextState(static_cast<int>(Screens::StoreTakenEnding));
			}

			//Noguest
			else
			{
				Engine::GetGameStateManager().SetNextState(static_cast<int>(Screens::NoGuestEnding));
			}
		}
		...
		{
			Engine::GetGameStateManager().SetNextState(static_cast<int>(Screens::BadEnding));
		}
	}
