...
Icecream::Icecream(rect3 rect, Player* _player, FoodType _type)
    : WorkBench(rect, _player), type(_type)
{
    ...
    texture = Engine::GetTextureManager().Load("assets/images/map/icecream_machine.png", false);
...
void Icecream::Update([[maybe_unused]] double dt)
{
	deltaTime = dt;
	if (Engine::GetGSComponent<DayByDay>()->GetDrawbool() == false)
	{
		...
		{
			if (Engine::GetGSComponent<Unlock>()->GetIcecream() == true)
			{
				isDraw = true;
				...
				if (timer > 1.5)
				{
					...
					numPos = GetPosition();
					food->SetType(type);
					Engine::GetGSComponent<Number>()->UpdateNumber(-3);
					player->SetFood(*food);
					timer = 0;
				}
			}
